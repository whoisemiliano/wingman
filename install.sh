...
if [ "$PLATFORM" != "windows" ]; then
    ...
    mkdir -p "$BIN_DIR"
    ln -sf "$INSTALL_DIR_ABS/$EXECUTABLE" "$BIN_DIR/wingman"

    # Ensure ~/.local/bin is in PATH so 'wingman' is found
    ...
    if [ -n "$RC" ] && [ -f "$RC" ]; then
        if ! grep -q '.local/bin' "$RC" 2>/dev/null; then
            ...
            echo "# Wingman CLI" >> "$RC"
            echo "$PATH_ADD" >> "$RC"
            print_success "Added ~/.local/bin to PATH in $RC"
        fi
    fi
    # Apply for current session so 'wingman' works without reopening terminal
...
if [ "$PLATFORM" != "windows" ]; then
    print_status "Run: wingman --help (if you get 'command not found', open a new terminal or run: source ~/.zshrc)"
fi
